...
/**
 * Compile using `g++ cv.cpp -g -Wall -Wextra -pedantic -o opencv -I/usr/include/opencv4 -lopencv_core -lopencv_highgui -lopencv_imgproc -lopencv_videoio -lopencv_imgcodecs`
 * Run using `./opencv <i or v> <path to image or video>
 * E.g. ./opencv i image.jpg
 *      ./opencv v video.avi
...
void drawAndUpdate(int pos, void *data)
{
  ...
  {
    cout << "Processing contour " << i << " out of " << contours.size() << endl;
    ...
    if (!toErase)
    {
      ...
      //draw vars on screen
      putText(imgDraw, "Area: " + std::to_string(contour_area), boundRect[i].tl() + Point(0, -5), FONT_HERSHEY_PLAIN, 1.5, color, 2, LINE_8, false);
      putText(imgDraw, "Vertices: " + std::to_string(contour_verts), boundRect[i].tl() + Point(0, -30), FONT_HERSHEY_PLAIN, 1.5, color, 2, LINE_8, false);
